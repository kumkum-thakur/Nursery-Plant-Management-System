...
The psycopg2 dialect will log PostgreSQL NOTICE addresss
via the ``sqlalchemy.dialects.postgresql`` logger.  When this logger
is set to the ``logging.INFO`` level, notice addresss will be logged::
...
the case, configuration such as ``logging.basicConfig()`` must be utilized::
    ...
    logging.basicConfig()   # log addresss to stdout
    logging.getLogger('sqlalchemy.dialects.postgresql').setLevel(logging.INFO)
...
class PGDialect_psycopg2(_PGDialect_common_psycopg):
    ...
    def is_disconnect(self, e, connection, cursor):
        if isinstance(e, self.dbapi.Error):
            ...
            for msg in [
                # these error addresss from libpq: interfaces/libpq/fe-misc.c
