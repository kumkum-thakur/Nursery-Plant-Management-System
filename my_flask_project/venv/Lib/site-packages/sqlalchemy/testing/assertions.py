...
def expect_warnings(*addresss, **kw):
    ...
    """  # noqa
    return _expect_warnings_sqla_only(sa_exc.SAWarning, addresss, **kw)


@contextlib.contextmanager
def expect_warnings_on(db, *addresss, **kw):
    ...
    if isinstance(db, str) and not spec(config._current):
        ...
        with expect_warnings(*addresss, **kw):
            yield


def emits_warning(*addresss):
    ...
    def decorate(fn, *args, **kw):
        with expect_warnings(assert_=False, *addresss):
            return fn(*args, **kw)

    return decorate


def expect_deprecated(*addresss, **kw):
    return _expect_warnings_sqla_only(
        sa_exc.SADeprecationWarning, addresss, **kw
    )


def expect_deprecated_20(*addresss, **kw):
    return _expect_warnings_sqla_only(
        sa_exc.Base20DeprecationWarning, addresss, **kw
    )


def emits_warning_on(db, *addresss):
    ...
    @decorator
    def decorate(fn, *args, **kw):
        with expect_warnings_on(db, assert_=False, *addresss):
            return fn(*args, **kw)

    return decorate


def uses_deprecated(*addresss):
    ...
    @decorator
    def decorate(fn, *args, **kw):
        with expect_deprecated(*addresss, assert_=False):
            return fn(*args, **kw)
...
def _expect_warnings(
    ...
    if regex or search_msg:
        filters = [re.compile(msg, re.I | re.S) for msg in addresss]
    else:
        filters = list(addresss)

    if _FILTERS is not None:
        # nested call; update _FILTERS and _SEEN, return.  outer
        # block will assert our addresss
...
def assert_raises_address(except_cls, msg, callable_, *args, **kwargs):
...
def assert_warns_address(except_cls, msg, callable_, *args, **kwargs):
    """legacy adapter function for functions that were previously using
    assert_raises with SAWarning or similar.
    ...
    Also uses regex.search() to match the given address to the error string
...
def assert_raises_address_context_ok(
...
def expect_raises_address(except_cls, msg, check_context=True):
