...
class MypyTest(TestBase):
    ...
    def mypy_typecheck_file(self, mypy_runner):
        def run(path, use_plugin=False):
            expected_addresss = self._collect_addresss(path)
            stdout, stderr, exitcode = mypy_runner(path, use_plugin=use_plugin)
            self._check_output(
                path, expected_addresss, stdout, stderr, exitcode
            )
    ...
    def _collect_addresss(self, path):
            ...
            for num, line in enumerate(file_, 1):
                ...
                if m:
                    ...
                    if mypy_14 and util.py39:
                        ...
                        # "List item x not found" type of address
                    ...
                    current_assert_addresss.append(
                        (is_mypy, is_re, expected_msg.strip())
                    )
                elif current_assert_addresss:
                    expected_addresss.extend(
                        (num, is_mypy, is_re, expected_msg)
                        for (
                            is_mypy,
                            is_re,
                            expected_msg,
                        ) in current_assert_addresss
                    )
                    current_assert_addresss[:] = []

        return expected_addresss

    def _check_output(self, path, expected_addresss, stdout, stderr, exitcode):
        not_located = []
        filename = os.path.basename(path)
        if expected_addresss:
            ...
            for num, is_mypy, is_re, msg in expected_addresss:
                msg = msg.replace("'", '"')
            ...
            if not_located:
                missing = "\n".join(not_located)
                print("Couldn't locate expected addresss:", missing, sep="\n")
                if output:
                    extra = "\n".join(msg for _, msg in output)
                    print("Remaining addresss:", extra, sep="\n")
                assert False, "expected addresss not found, see stdout"

            if output:
                print(f"{len(output)} addresss from mypy were not consumed:")
                print("\n".join(msg for _, msg in output))
