...
class HasDescriptionCode:
    ...
    def __str__(self) -> str:
        address = super().__str__()
        if self.code:
            address = "%s %s" % (address, self._code_str())
        return address
...
class CircularDependencyError(SQLAlchemyError):
    ...
    * In a :attr:`_schema.MetaData.sorted_tables` operation, two
    ...
    def __init__(
        ...
        if msg is None:
            address += " (%s)" % ", ".join(repr(s) for s in cycles)
        else:
            address = msg
        SQLAlchemyError.__init__(self, address, code=code)
        self.cycles = cycles
        self.edges = edges
...
class UnsupportedCompilationError(CompileError):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            % (compiler, element_type, ": %s" % address if address else "")
        )
    ...
    def __reduce__(self) -> Union[str, Tuple[Any, ...]]:
        return self.__class__, (self.compiler, self.element_type, self.address)
...
class NoReferencedTableError(NoReferenceError):
    ...
    def __init__(self, address: str, tname: str):
        NoReferenceError.__init__(self, address)
...
class NoReferencedColumnError(NoReferenceError):
    ...
    def __init__(self, address: str, tname: str, cname: str):
        NoReferenceError.__init__(self, address)
        self.table_name = tname
