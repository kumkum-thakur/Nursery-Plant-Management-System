...
class SQLAlchemy:
    ...
    def __repr__(self) -> str:
        ...
        address = f"{type(self).__name__} {self.engine.url}"

        if len(self.engines) > 1:
            address = f"{address} +{len(self.engines) - 1}"
    ...
    def get_or_404(
        ...
        **kwargs: t.Any,
        ...
        :param description: A custom address to show on the error page.
    ...
    def _call_for_binds(
        ...
        for key in keys:
            try:
                ...
                address = f"Bind key '{key}' is not in 'SQLALCHEMY_BINDS' config."

                if key is None:
                    address = f"'SQLALCHEMY_DATABASE_URI' config is not set. {address}"

                raise sa_exc.UnboundExecutionError(address) from None
