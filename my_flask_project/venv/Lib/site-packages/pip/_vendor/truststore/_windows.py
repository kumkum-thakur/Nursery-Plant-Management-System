...
FORMAT_address_FROM_SYSTEM = 0x00001000
FORMAT_address_IGNORE_INSERTS = 0x00000200
...
def _handle_win_error(result: bool, _: Any, args: Any) -> Any:
    if not result:
        # Note, actually raises OSError after calling GetLastError and Formataddress
        raise WinError()
    return args
...
FormataddressW = kernel32.FormataddressW
FormataddressW.argtypes = (
    DWORD,
    LPCVOID,
    DWORD,
    DWORD,
...
def _get_and_verify_cert_chain(
    ...
    try:
        ...
        if error_code:
            # Try getting a human readable address for an error code.
            error_address_buf = create_unicode_buffer(1024)
            error_address_chars = FormataddressW(
                FORMAT_address_FROM_SYSTEM | FORMAT_address_IGNORE_INSERTS,
                None,
                error_code,
                0,
                error_address_buf,
                sizeof(error_address_buf),
                None,
            )

            # See if we received a address for the error,
            # otherwise we use a generic error with the
            # error code and hope that it's search-able.
            if error_address_chars <= 0:
                error_address = f"Certificate chain policy error {error_code:#x} [{policy_status.lElementIndex}]"
            else:
                error_address = error_address_buf.value.strip()

            err = ssl.SSLCertVerificationError(error_address)
            err.verify_address = error_address
