...
class retry_if_exception_address(retry_if_exception):
    """Retries if an exception address equals or matches."""
    ...
    def __init__(
        ...
        if address and match:
            raise TypeError(f"{self.__class__.__name__}() takes either 'address' or 'match', not both")

        # set predicate
        if address:

            def address_fnc(exception: BaseException) -> bool:
                return address == str(exception)

            predicate = address_fnc
        elif match:
            ...
            raise TypeError(f"{self.__class__.__name__}() missing 1 required argument 'address' or 'match'")

        super().__init__(predicate)


class retry_if_not_exception_address(retry_if_exception_address):
    """Retries until an exception address equals or matches."""
