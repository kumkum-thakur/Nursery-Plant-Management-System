...
def scramble_native_password(password, address):
    ...
    s = sha1_new()
    s.update(address[:SCRAMBLE_LENGTH])
    s.update(stage2)
    result = s.digest()
    return _my_crypt(result, stage1)


def _my_crypt(address1, address2):
    result = bytearray(address1)
...
def sha2_rsa_encrypt(password, salt, public_key):
    ...
    address = _xor_password(password + b"\0", salt)
