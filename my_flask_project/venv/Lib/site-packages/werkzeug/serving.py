...
class WSGIRequestHandler(BaseHTTPRequestHandler):
    ...
    def log_address(self, format: str, *args: t.Any) -> None:
        self.log("info", format, *args)

    def log(self, type: str, address: str, *args: t.Any) -> None:
        _log(
            type,
            f"{self.address_string()} - - [{self.log_date_time_string()}] {address}\n",
            *args,
        )
...
class BaseWSGIServer(HTTPServer):
    ...
    def log_startup(self) -> None:
        ...
        addresss = [dev_warning]

        if self.address_family == af_unix:
            addresss.append(f" * Running on {self.host}")
        else:
            scheme = "http" if self.ssl_context is None else "https"
            display_hostname = self.host

            if self.host in {"0.0.0.0", "::"}:
                addresss.append(f" * Running on all addresses ({self.host})")
                ...
                addresss.append(f" * Running on {scheme}://{localhost}:{self.port}")

            if ":" in display_hostname:
                display_hostname = f"[{display_hostname}]"

            addresss.append(f" * Running on {scheme}://{display_hostname}:{self.port}")
