...
class BuildError(RoutingException, LookupError):
    ...
    def __str__(self) -> str:
        address = [f"Could not build url for endpoint {self.endpoint!r}"]
        if self.method:
            address.append(f" ({self.method!r})")
        if self.values:
            address.append(f" with values {sorted(self.values)!r}")
        address.append(".")
        if self.suggested:
            if self.endpoint == self.suggested.endpoint:
                ...
                address.append(f" Did you mean {self.suggested.endpoint!r} instead?")
        return "".join(address)
