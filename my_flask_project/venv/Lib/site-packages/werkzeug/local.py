...
class Local:
    ...
    def __call__(
        self, name: str, *, unbound_address: str | None = None
    ) -> LocalProxy[t.Any]:
        """Create a :class:`LocalProxy` that access an attribute on this
        local namespace.

        :param name: Proxy this attribute.
        :param unbound_address: The error address that the proxy will
            show if the attribute isn't set.
        """
        return LocalProxy(self, name, unbound_address=unbound_address)
...
class LocalStack(t.Generic[T]):
    ...
    def __call__(
        self, name: str | None = None, *, unbound_address: str | None = None
    ) -> LocalProxy[t.Any]:
...
class LocalProxy(t.Generic[T]):
    ...
    :param unbound_address: The error address to show if the
    ...
    .. versionchanged:: 2.2
        Added the ``unbound_address`` parameter.
    ...
    def __init__(
        ...
        *,
        ...
        if unbound_address is None:
            unbound_address = "object is not bound"
        ...
        if isinstance(local, Local):
            ...
            def _get_current_object() -> T:
                try:
                    ...
                    raise RuntimeError(unbound_address) from None
            ...
            def _get_current_object() -> T:
                ...
                if obj is None:
                    raise RuntimeError(unbound_address)
